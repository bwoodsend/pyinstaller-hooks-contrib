...
**Please provide one hook per pull request! If you have more than one then submit them in separate pull requests.**
...
*   Long lists of `hiddenimport` submodules.
    If you really need lots of hidden imports then use [`collect_submodules('foo')`](https://pyinstaller.readthedocs.io/en/latest/hooks.html#PyInstaller.utils.hooks.collect_submodules).
...
*   Use of [`collect_all()`](https://pyinstaller.readthedocs.io/en/latest/hooks.html#PyInstaller.utils.hooks.collect_all).
    This function is extremely inefficient and is really only for emergencies.
...
*   Any complicated `os.path` arithmetic (by which I mean overly complex filename manipulations).
...
With them we can automatically verify that your hook works on all platforms, all Python versions and new versions of libraries as they are released.
Without them, we have no idea if the hook is broken until someone finds out the hard way.
Please write tests!!!

That said, some user interface libraries may be impossible to test without user interaction
...
Instead explain in the commit message why an automatic test is impractical then skip on to [the next step](#run-linter).
...
Good sources of such minimal tests are the hello world examples
...
If need be, you can specify version constraints using `@importorskip('foo', minversion="1.2.3")`
...
the full test suite runs like a dead snail on ketamine.
...
We'll eventually want to see a build (or collection of builds) which passes on
...
You'll still have to enter all the details again but, thanks to terminal history,
